...
const Partners = () => {
  return (
    <div className='my-20'>
      <h1 className='font-bold text-deep-blue text-4xl text-center mb-5'>Partners <span className='text-light-yellow'>and Networks</span></h1>
      <div className='flex flex-col gap-8'>
        {
            LIST_PARTNERS.map((partner) => (
                <div key={partner.cluster} className='flex flex-col gap-4'>
                    <h3 className='font-bold text-lg text-center'>{partner.cluster}</h3>
                    <Slider {...settings}>
                        {partner.list.map((srcAssets, index) => (
                            <div key={index} className="mx-8">
                                <img src={srcAssets} alt={`Maida ${partner.cluster} Partner`} className="mx-auto max-h-[5rem]" />
                            </div>
                        ))}
                    </Slider>
                </div>
